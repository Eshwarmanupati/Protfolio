...
- **Smooth page transitions** — pages crossfade with a small directional drift while the nav, footer and reading rail stay put, so the site reads as one continuous surface. Opacity and transform only, which keeps it at frame rate. Browsers without view transitions fall back to a JS curtain wipe
...
The animation is **only opacity and transform**. Those are the two properties the compositor can
animate without laying out or repainting, which is what keeps a full-page snapshot at frame rate.
A blur or an SVG filter here costs a repaint every frame and is what makes a transition feel heavy
— if you are tempted to add one, that is the trade you are making.

Two details worth keeping if you tune it:

- The drift is 14–18px and half a percent of scale. Enough to give the change a direction, not
  enough to read as a slide.
- In `pageIn`, opacity finishes at the 55% mark while the drift keeps easing out behind it. If both
  ran the full duration the two pages would sit half-transparent over each other at the midpoint
  and the whole thing would look washed out.
